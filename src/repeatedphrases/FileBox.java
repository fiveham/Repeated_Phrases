...
public class FileBox{
	...
	/*public void removeUniques(){
		...
		//For each key in the map, if there's only one Location in the key's 
		//associated list, then remove that mapping
		...
		System.out.printf("Removed %d non-repeated terms, taking %s\n", 
				initSize-size(), FindRepeatedPhrases.timeSince(initTime));
	}//**
	
	/*private void removeAndPrintUniques(PrintStream uniq){ //FIX ME debugging purposes only
		final long initTime = System.currentTimeMillis();
		final int initSize = size();
		
		//For each key in the map, if there's only one Location in the key's 
		//associated value, then remove that mapping
		List<String> keys = new ArrayList<>(hashmap.keySet());
		for(String phrase : keys ){
			if(hashmap.get(phrase).size() <= FindRepeatedPhrases.UNIQUE_PHRASE_LOCATION_COUNT){
				List<Location> locs = hashmap.remove(phrase);
				uniq.print(phrase);
				for(Location l : locs){
					uniq.print(PhraseProducer.WORD_SEPARATOR);
					uniq.print(l.toString());
				}
				uniq.println();
			}
		}
	...
	/*public void printPhrasesWithLocations(PrintStream repeatedPhrases){
		
		for(String filename : hashmap.keySet()){
			repeatedPhrases.print( filename );
			List<Location> list = hashmap.get(filename);
			list.sort(null);
			for( int i=0; i<list.size(); i++ ){
				repeatedPhrases.printf( "%s%s", IO.LOCATION_DELIM, list.get(i).toString() );
			}

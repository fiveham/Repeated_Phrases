...
public class RemoveDependentPhrases {
	...
	/**
	 * <p>An exclusive lower bound for how small the smaller of two phrase-sizes 
	 * being used at once for dependence-testing can be. No phrase can be of this 
	 * size ({@value}).</p>
	 */
	...
	/**
	 * <p>Iterates over a set of pre-existing files generated by 
	 * FindRepeatedPhrases, starting at the largest phrase-size 
	 * for which the file output by FindRepeatedPhrases is not 
	 * empty.
	 * 
	 * For each phrase-size considered, the 
	 * repeated-phrase data for that size and for that size 
	 * plus 1 are loaded into memory from the pertinent pre-
	 * existing files created by FindRepeatedPhrases, except 
	 * that the data for the previous size is carried over 
	 ...
	 * is called, ending the program.</p>

...
/**
 * <p>This class is the main class in the jar from which this 
 * application is launched; it launches the GUI as a new process 
 * with an enhanced maximum heap size of 512 megabytes allowing the 
...
public class Launch {
	...
	/**
	 * <p></p>
	 * <p>Determines the name of the only jar file in the working directory 
	 * (the jar which the user double-clicked to run this class), or, if there 
	 * are multiple jar files, assumes that the correct name is "repeatedphrases.jar", 
	 * the name under which the jar for this program was released. Launches 
	 * {@link RepeatedPhrasesUI the GUI} as 
	 * {@link Runtime#exec(String) a new process} with java option {@value #MAX_HEAP} 
	 * allowing repeated-phrase analysis to complete.</p>
